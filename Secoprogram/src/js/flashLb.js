     function flashLunbo(opt) {
       ...
       let playimages = document.getElementById(defaultOpt.ele);
       //设置尺寸
       playimages.style.width = defaultOpt.iw + 'px';
       playimages.style.height = defaultOpt.ih + 'px';
       console.log(playimages.style.width);
    ...
    let html2 = '';
    defaultOpt.datalist.forEach((item) => {
        html2 += `<li><img src="${item}" /></li>`;
      });

    smallUl .innerHTML =  html2;
    let iW = smallUl.children[0].offsetWidth;
    smallUl.children[0].classList.add('active');
       ...
       function tab() { //切换到第now张图
         ...
         startMove(smallUl.children[now], {
           'opacity': 100
         });
       }

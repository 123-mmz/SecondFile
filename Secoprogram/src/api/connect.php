<?php
    ...
    // /* 执行sql语句查询结果集 */
    $sql = "SELECT * FROM cars WHERE id='$uid' AND tel='$tel'";
    $res = $conn->query($sql);//$res 是一个结果集

    //执行语句
    if($res->num_rows){
      $sql2="UPDATE cars SET num='$num',colors='$colors' WHERE id='$uid' AND tel='$tel'";
    
    }else{
        $sql2="INSERT INTO cars(num,colors,id,tel) VALUES ('$num','$colors','$uid','$tel')";
    }
